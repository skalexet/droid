...
export default class TileCollider { //esp. to Level.js
	constructor() {
		this.resolvers = [];
	}
	...
	checkX(entity, gameContext, level) { //to check position of given entity
		let x; // x
		if (entity.vel.x > 0) { 
			x = entity.bounds.right; // x takes right corner of entity
		} else if (entity.vel.x < 0) { 
			x = entity.bounds.left; // x takes left corner
		} else {
			return; 
		}
		
		for (const resolver of this.resolvers) {
			const matches = resolver.searchByRange( // inner meth of Resolver that returns
				x, x, 								 //array of objects with position 
	...
	checkY(entity, gameContext, level) {
		let y;
		if (entity.vel.y > 0) {
			y = entity.bounds.bottom; //same as above
		...
		for (const resolver of this.resolvers) {		
	    	const matches = resolver.searchByRange( //same returns array of objs with pos
